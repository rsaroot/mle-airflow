...
def clean_churn_dataset():
    ...
    def extract():
        ...
        data = pd.read_sql(sql, conn, parse_dates=['start_date', 'end_date']).drop(columns=['id'])
        conn.close()
        return data
